...
import {
    LayoutDashboard, Package, Settings, Bell, LogOut, Users, Truck,
    CheckCheck, Receipt, ShoppingCart, BarChart3, BellOff, ShoppingBag,
    Mail, X, ArrowRight, BellRing, Menu
} from 'lucide-react';
...
type BellTab = 'all' | 'INVENTORY' | 'BILLING' | 'SYSTEM';
        ...
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-96 p-0 bg-slate-900 border-white/10 shadow-2xl mr-4 rounded-2xl overflow-hidden" align="end" sideOffset={8}>
                        ...
                        return (
                            <div key={n.id}
                                ...
                                <div className="mt-1.5 shrink-0">
                                    <div className={`w-1.5 h-1.5 rounded-full ${!n.isRead ? pc.dot : 'bg-transparent'}`} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className={`text-xs font-bold leading-snug ${!n.isRead ? 'text-white' : 'text-slate-300'}`}>
                                        {n.title}
                                    </p>
                                    ...
                                    <div className="flex items-center gap-2 mt-1.5">
                                        <span className="text-[9px] text-slate-600 uppercase tracking-widest font-bold">
                                            {getCategoryConfig(n.category).emoji} {getCategoryConfig(n.category).label}
                                        </span>
                                        <span className="text-[9px] text-slate-700">·</span>
                                        <span className="text-[9px] text-slate-600">{formatTimestamp(new Date(n.timestamp))}</span>
                                        {n.emailSent && (
                                            <>
                                                <span className="text-[9px] text-slate-700">·</span>
                                                <span className="text-[9px] text-emerald-500/70 flex items-center gap-0.5">
                                ...
                                <div className="flex flex-col items-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                    {!n.isRead && (
                                        <button onClick={() => markAsRead(n.id)}
                                            className="p-1 rounded hover:bg-indigo-500/10 text-slate-600 hover:text-indigo-400 transition-colors">
                                            <CheckCheck className="w-3 h-3" />
                                        </button>
                                    )}
                                    {n.actionUrl && (
                                        <button onClick={() => { router.push(n.actionUrl!); setOpen(false); }}
                ...
                <div className="px-5 py-3 border-t border-white/5 bg-black/20">
                    <button
                        onClick={() => { router.push('/store-manager/notifications'); setOpen(false); }}
...
const StoreTopBar = ({ onMenuTrigger }: { onMenuTrigger?: () => void }) => {
    ...
    return (
        <header className="h-20 border-b border-white/5 bg-slate-950/50 backdrop-blur-3xl flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30">
            <div className="flex items-center gap-4">
                <div>
                    <h2 className="text-xl font-black text-white italic tracking-wide">STORE <span className="text-indigo-400 not-italic">COMMAND</span></h2>
                    <p className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Live Inventory Management</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3 border-l border-white/10 pl-4">
                    ...
                    <div className="hidden md:block mr-2">
                        <p className="text-sm font-bold text-white leading-none">{user?.name || 'Store Manager'}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{user?.role?.replace('_', ' ') || 'MANAGER'}</p>
                    </div>
                    <button onClick={logout}
                        className="p-2 rounded-full bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 hover:text-rose-300 transition-colors" title="Sign Out">
...
export default function StoreManagerLayout({ children }: { children: React.ReactNode }) {
    ...
    React.useEffect(() => {
        ...
        else if (!isLoading && user && user.role !== 'STORE_MANAGER' && user.role !== 'ADMIN') { router.push('/member'); }
    }, [user, isLoading, router]);

    const navItems = [
        { name: 'Dashboard', icon: LayoutDashboard, path: '/store-manager' },
        { name: 'Analytics', icon: BarChart3, path: '/store-manager/analytics' },
        { name: 'Inventory', icon: Package, path: '/store-manager/inventory' },
        { name: 'Orders', icon: ShoppingBag, path: '/store-manager/orders' },
        { name: 'Billing', icon: Receipt, path: '/store-manager/billing' },
        { name: 'Procurement', icon: Truck, path: '/store-manager/procurement' },
        { name: 'Customers', icon: Users, path: '/store-manager/customers' },
        { name: 'Settings', icon: Settings, path: '/store-manager/settings' },
    ];

    if (isLoading || !user) return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white font-black italic">
            LOADING STORE COMMAND...
        </div>
    );
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-slate-950 text-slate-50 selection:bg-indigo-500/30">
            ...
            <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
                <StoreTopBar onMenuTrigger={() => setIsMobileMenuOpen(true)} />
                        ...
                        if (item.isMore) {
                            return (
                                <button
                                    ...
                                    className="flex flex-col items-center justify-center flex-1 h-full py-2 relative transition-all duration-350 text-slate-500 hover:text-slate-350"
                                >
            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
                <SheetContent side="left" className="p-0 w-72 bg-slate-950 border-r border-white/5 text-slate-50">
                    <div className="sr-only">
                        ...
                        <SheetDescription>Navigation links for the store manager portal</SheetDescription>
                    </div>
            ...
            <Sheet open={isBottomMenuOpen} onOpenChange={setIsBottomMenuOpen}>
                <SheetContent side="bottom" className="p-0 rounded-t-[2rem] border-t border-white/5 bg-slate-950 text-slate-50 max-h-[85vh] overflow-hidden flex flex-col">
                    <div className="sr-only">
                        ...
                        <SheetDescription>Bottom sheet navigation links for the store manager portal</SheetDescription>
                    </div>
                    <div className="flex flex-col h-full bg-black/40 backdrop-blur-2xl">
                        <div className="px-6 pt-6 pb-4 flex justify-between items-center">
                            <div>
                                <span className="font-heading font-black text-lg tracking-wider text-white uppercase italic">
                                    EXPLORE <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">STORE</span>
                                </span>
