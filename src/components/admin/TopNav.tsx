...
import { Bell, Search, User, LogOut, CreditCard, Dumbbell, Megaphone, Clock, CheckCheck, Flame, Menu } from 'lucide-react';
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            addNotification({
                type: 'ANNOUNCEMENT',
                title: 'System Alert',
                message: 'Daily automated database backup completed successfully.',
            });
            ...
            <div className="flex items-center gap-4 flex-1">
                ...
                <div className="relative w-full max-w-md group hidden md:block">
                    ...
                    <input
                        type="text"
                        placeholder="Search..."
                        className="w-full bg-slate-100 dark:bg-black/20 border border-slate-200 dark:border-primary/20 rounded-2xl py-2 pl-11 pr-4 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-muted-foreground/50"
                    />
                </div>
            </div>
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className="relative p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors text-muted-foreground hover:text-foreground outline-none">
                            ...
                            {unreadCount > 0 && (
                                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-destructive flex items-center justify-center shadow-[0_0_8px_hsl(var(--destructive))]"></span>
                            )}
                        </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-80 sm:w-96 p-0 border-slate-200 dark:border-primary/20 bg-background/95 backdrop-blur-xl">
                        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-primary/10">
                            <DropdownMenuLabel className="font-semibold text-lg p-0">Admin Notifications</DropdownMenuLabel>
                            {unreadCount > 0 && (
                                <Button
                                    variant="ghost"
                                    size="sm"
                            ...
                            {notifications.length === 0 ? (
                                ...
                                <div className="flex flex-col">
                                    {notifications.map((notification) => (
                                        ...
                                        >
                                            <div className="mt-0.5 p-2 rounded-full bg-background border border-slate-100 dark:border-primary/20 flex-shrink-0">
